...
class DeleteSkill extends React.Component{
    ...
    async getSkills(candidateId){
        InteNSAxios.get("/skills/candidatehas/" + candidateId)
        .then(res => {
...
removeSkill(){
    ...
    InteNSAxios.put('/candidates/deleteskill/'+ this.state.candidate.id, params)
    .then(res=> {
        window.location.reload();
    } 
    ).catch(error => alert("Error, skill not removed"))
    ...
    render() {
        return (
            ...
            <Form style={{ width: "99%" }}>
                <Row>
                <Form.Group>
                    <Form.Label>Remove skill for candidate: {this.state.candidate.fullName}</Form.Label>
            ...
            <Row><Col>
                <Button className="mt-3" onClick={() => this.removeSkill()}>Remove skill</Button>
            </Col></Row>
            </>
        );
    }
...
export default withNavigation(withParams(DeleteSkill));